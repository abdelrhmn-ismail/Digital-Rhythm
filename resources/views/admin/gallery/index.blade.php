...
<div class="p-6">
    ...
    <!-- Gallery Table -->
    <x-admin.table :headers="[__('Image Info'), __('Category'), __('Status'), __('Order')]" :items="$galleryImages">
            ...
            <tr class="hover:bg-gray-50 transition-all duration-200">
                <td class="px-6 py-4">
                    <div class="flex items-start gap-4">
                        ...
                        @endif
                        <div class="flex-1">
                            <div class="font-bold text-gray-900 leading-tight">{{ $image->title ?: __('Untitled') }}</div>
                            <div class="text-[10px] text-muted mt-0.5 font-mono uppercase tracking-wider">{{ __('Uploaded') }}: {{ $image->created_at->format('M d, Y') }}</div>
                            @if($image->caption)
                                <div class="text-[10px] text-gray-500 mt-1 line-clamp-1 border-t border-gray-100 pt-1">{{ Str::limit(strip_tags($image->caption), 60) }}</div>
                            @endif
                        </div>
                    </div>
                </td>
                <td class="px-6 py-4">
                    <div class="text-sm font-medium text-gray-900">{{ $image->category ?: __('No Category') }}</div>
                    @if($image->tags)
                        <div class="text-[9px] text-gray-500 mt-1 flex flex-wrap gap-1">
                            @foreach(explode(',', $image->tags) as $tag)
                                <span class="bg-gray-100 px-1.5 rounded">#{{ trim($tag) }}</span>
                            @endforeach
                        </div>
                    @endif
                </td>
                <td class="px-6 py-4">
                    <div class="flex flex-col gap-1">
                        ...
                        @endif
                        <x-admin.status-badge type="active" :value="$image->is_active" />
                    </div>
                </td>
                ...
                <td class="px-6 py-4 text-right">
                    <div class="flex items-center gap-3">
                        ...
                        <button onclick="toggleFeatured('gallery', {{ $image->id }})" 
                            ...
                            <span class="material-icons text-lg">{{ $image->is_featured ? 'auto_awesome' : 'auto_awesome_motion' }}</span>
                        </button>
                        ...
                        <button onclick="toggleActive('gallery', {{ $image->id }})" 
                            ...
                            <span class="material-icons text-lg">{{ $image->is_active ? 'visibility' : 'visibility_off' }}</span>
                        </button>
                        
                        <form method="POST" action="{{ route('admin.gallery.destroy', $image) }}" 
                              onsubmit="return confirm('{{ __('Are you sure you want to delete this image?') }}')" class="inline">
                            @csrf
                            @method('DELETE')
