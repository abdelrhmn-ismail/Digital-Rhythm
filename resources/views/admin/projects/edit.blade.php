...
<div x-data="{ tab: 'en' }" class="max-w-6xl mx-auto pb-24">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
            ...
            <h1 class="text-4xl font-black text-gray-900 tracking-tight">{{ __('Edit Project') }}: {{ $project->title }}</h1>
        </div>
                        ...
                        <div x-show="tab === 'en'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0">
                            ...
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <input type="text" name="title[en]" value="{{ old('title.en', $project->getTranslation('title', 'en', false)) }}" required 
                                ...
                                <div>
                                    ...
                                    <input type="text" name="client[en]" value="{{ old('client.en', $project->getTranslation('client', 'en', false)) }}"
                                ...
                                <div>
                                    ...
                                    <textarea name="description[en]" class="tinymce w-full min-h-[300px]">{{ old('description.en', $project->getTranslation('description', 'en', false)) }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div x-show="tab === 'ar'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 -translate-x-4" x-transition:enter-end="opacity-100 translate-x-0" dir="rtl">
                            ...
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <input type="text" name="title[ar]" value="{{ old('title.ar', $project->getTranslation('title', 'ar', false)) }}" required 
                                ...
                                <div>
                                    ...
                                    <input type="text" name="client[ar]" value="{{ old('client.ar', $project->getTranslation('client', 'ar', false)) }}"
                                ...
                                <div>
                                    ...
                                    <textarea name="description[ar]" class="tinymce w-full min-h-[300px] text-right font-alexandria">{{ old('description.ar', $project->getTranslation('description', 'ar', false)) }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="space-y-8">
                ...
                <div class="bg-primary/5 rounded-[40px] p-8 border border-primary/10 space-y-8">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="completed_date" value="{{ old('completed_date', $project->completed_date ? $project->completed_date->format('Y-m-d') : '') }}"
                                class="w-full px-5 py-3 rounded-2xl bg-white border-transparent focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all outline-none text-sm font-bold shadow-sm">
                        </div>
                    ...
                    <div class="pt-8 border-t border-primary/10 space-y-4">
                        ...
                        <label class="flex items-center justify-between cursor-pointer group">
                            ...
                            <div class="relative w-12 h-6 rounded-full bg-gray-200 group-hover:bg-gray-300 transition-colors">
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $project->is_active) ? 'checked' : '' }} class="sr-only peer">
                                <div class="peer-checked:translate-x-6 peer-checked:bg-primary absolute left-1 top-1 w-4 h-4 rounded-full bg-white transition-all shadow-sm"></div>
                            </div>
                        </label>
                    </div>
                ...
                <div class="bg-white rounded-[40px] p-8 shadow-sm border border-gray-100 space-y-6">
                    ...
                    <div class="relative py-8 rounded-[24px] bg-gray-50 border-2 border-dashed border-gray-100 flex flex-col items-center justify-center text-center px-4 hover:border-primary/20 transition-all cursor-pointer group"
                         x-data="{ previewUrl: '{{ $project->image_url }}' }" @change="const file = $event.target.files[0]; if (file) { previewUrl = URL.createObjectURL(file) }">
                        ...
                        <div x-show="previewUrl" class="relative w-full h-40 rounded-xl overflow-hidden shadow-sm">
                ...
                <div class="bg-white rounded-[40px] p-8 shadow-sm border border-gray-100 space-y-6">
                    ...
                    <!-- Previews of existing gallery -->
                    @if($project->images && count($project->images) > 0)
                        <div class="grid grid-cols-3 gap-2 mb-4">
                            @foreach($project->images_urls as $image)
                                <div class="relative h-16 rounded-lg overflow-hidden border border-gray-200 shadow-sm">
                                    <img src="{{ $image }}" class="w-full h-full object-cover">
                                </div>
                            @endforeach
                        </div>
                        <p class="text-[10px] text-gray-400 italic mb-4">{{ __('Uploading new images will replace the entire gallery.') }}</p>
                    @endif

                    <div class="relative py-8 rounded-[24px] bg-gray-50 border-2 border-dashed border-gray-100 flex flex-col items-center justify-center text-center px-4 hover:border-primary/20 transition-all cursor-pointer group">
                        <span class="material-icons text-3xl text-gray-200 group-hover:text-primary/20 transition-colors mb-2">collections</span>
                        <span class="text-[10px] font-black text-gray-300 uppercase tracking-widest group-hover:text-gray-400 transition-colors">{{ __('Upload New Gallery Images') }}</span>
