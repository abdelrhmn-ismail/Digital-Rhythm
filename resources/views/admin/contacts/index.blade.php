...
<div class="space-y-6">
    <div class="flex flex-col gap-4 rounded-2xl bg-white p-6 shadow-sm border border-gray-100 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <p class="text-sm text-gray-400">{{ __('Manage inquiries from your website visitors.') }}</p>
        </div>
    ...
    <form method="GET" class="grid gap-4 rounded-2xl bg-white p-4 shadow-sm border border-gray-100 md:grid-cols-4">
        <div class="md:col-span-2">
            ...
            <input type="text" name="search" value="{{ $filters['search'] ?? ' }}" placeholder="{{ __('Search by name, email or message') }}" class="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-amber-500 focus:outline-none" />
        </div>
        <div>
            <label class="text-xs font-semibold text-gray-500 uppercase tracking-wide">{{ __('Status') }}</label>
            <select name="status" class="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-amber-500 focus:outline-none">
        ...
        <div>
            ...
            <select name="date" class="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-amber-500 focus:outline-none">
        ...
        <div class="flex items-end justify-end gap-2 md:col-span-4">
            ...
            <button type="submit" class="rounded-lg bg-amber-500 px-4 py-2 text-sm font-semibold text-white">{{ __('Filter') }}</button>
        </div>
    </form>
    ...
    <x-admin.table :headers="[__('Sender'), __('Message Snippet'), __('Status'), __('Created'), __('Replies')]" :items="$messages">
        @forelse($messages as $message)
            <tr class="hover:bg-gray-50">
                <td class="px-6 py-4 text-sm">
                    ...
                    <p class="text-gray-400 text-xs">{{ $message->company ?? __('No Company') }}</p>
                </td>
                ...
                <td class="px-6 py-4 text-sm text-gray-500">
                    ...
                    @else
                        <span class="text-gray-400 text-xs">{{ __('No reply yet') }}</span>
                    @endif
                </td>
                <td class="px-6 py-4 text-sm">
        ...
        @empty
            <tr>
                <td colspan="6" class="px-6 py-8 text-center text-sm text-gray-400">{{ __('No messages found for the selected filters.') }}</td>
            </tr>
        @endforelse
    </x-admin.table>
</div>
